...
<div class="gift-detail-wrapper">
    ...
    {{-- Lucky Gift Info --}}
    @if($luckyGift)
    <div class="gift-lucky-card">
        <h3 class="gift-lucky-title">
            <i class="fa fa-star"></i> {{ __('Lucky Gift Settings') }}
        </h3>
        @php
            $percentages = $luckyGift->min_percentage ? explode(',', $luckyGift->min_percentage) : [];
        @endphp
        <div class="gift-lucky-grid">
            <div class="gift-lucky-item">
                <div class="gift-lucky-item-label">{{ __('Win Probability') }}</div>
                <div class="gift-lucky-item-value">{{ $luckyGift->win_probability ?? 0 }}%</div>
            </div>
            @if(isset($percentages[0]))
            <div class="gift-lucky-item">
                <div class="gift-lucky-item-label">{{ __('Min %') }}</div>
                <div class="gift-lucky-item-value">{{ $percentages[0] }}%</div>
            </div>
            @endif
            @if(isset($percentages[1]))
            <div class="gift-lucky-item">
                <div class="gift-lucky-item-label">{{ __('Mid %') }}</div>
                <div class="gift-lucky-item-value">{{ $percentages[1] }}%</div>
            </div>
            @endif
            @if(isset($percentages[2]))
            <div class="gift-lucky-item">
                <div class="gift-lucky-item-label">{{ __('Max %') }}</div>
                <div class="gift-lucky-item-value">{{ $percentages[2] }}%</div>
            </div>
            @endif
        </div>
    </div>
