    ...
    public function update(EditRoomRequest $request, $id)
    {
        try {
            ...
            if ($request->room_type) {
                if (! RoomCategory::query()->where('id', $request->room_type)->where('enable', 1)->exists()) {
            ...
            if ($request->room_class) {
                if (! RoomCategory::query()->where('id', $request->room_class)->where('enable', 1)->exists()) {
                    return Common::apiResponse(0, 'class not found', null, 404);
                }
                $room->room_type = $request->room_type;
            }
            $background_me = '';
