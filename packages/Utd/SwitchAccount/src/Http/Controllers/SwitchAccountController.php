...
namespace Utd\SwitchAccount\Http\Controllers;
...
use Utd\SwitchAccount\Transformers\AccountResource;
...
class SwitchAccountController extends Controller
{
    ...
    public function add_account0(Request $request)
    {
        ...
        try {
            $otherUser = $this->getOtherUser($request);
            if ($otherUser->id === $user->id) {
                return Common::apiResponse(0, 'can not add yourself', null, 422);
            }
    ...
    public function getOtherUser($request)
    {
        ...
        if (mb_strpos($bearerToken, '|') !== false) {
    ...
    public function switch_account(Request $request)
    {
        ...
        if (! $request->token) {
            return Common::apiResponse(0, 'token not valid', null, 422);
        }
        ...
        $data = [
            'current' => [
                ...
                'country' => @$currentUser->country ? [
                    ...
                    'iso' => mb_substr(@$currentUser->country->iso, 0, 2),
                ] : null,
            ],
            'other' => AccountResource::collection($accounts),
        ];
