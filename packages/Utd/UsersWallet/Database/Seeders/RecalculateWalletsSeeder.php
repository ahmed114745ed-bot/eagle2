                ...
                foreach ($salaries as $s) {
                    try {
                        ...
                        if ((float) $s->cut_amount > 0) {
                            $walletModel = \Utd\UsersWallet\Entities\UserWallet::firstOrCreate(['user_id' => $s->user_id]);
                            ...
                            if (abs($after - $before) > 1e-8) {
                                \Utd\UsersWallet\Entities\WalletLog::create([
            ...
            foreach ($rows as $a) {
                try {
                    if ((float) $a->cut_amount > 0) {
                        ...
                        $walletModel = \Utd\UsersWallet\Entities\UserWallet::firstOrCreate(['user_id' => $ownerId]);
                        $before = wallet_available_by_wallet($walletModel);
        ...
        \Utd\Bd\Entities\BdSalary::query()->orderBy('id')->chunk(200, function ($bdRows) {
            foreach ($bdRows as $b) {
                try {
                    ...
                    $walletModel = \Utd\UsersWallet\Entities\UserWallet::firstOrCreate(['user_id' => $userId]);
                    $before = wallet_available_by_wallet($walletModel);
