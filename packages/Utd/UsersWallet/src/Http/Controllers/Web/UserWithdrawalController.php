...
use Utd\UsersWallet\Entities\WalletField;
...
class UserWithdrawalController extends MainController
{
    ...
    protected function grid()
    {
        ...
        $grid->column('actions', __('Actions'))->display(function () {
            ...
            <div class="btn-group" role="group">
                <button class="btn btn-success btn-sm approve-btn" data-url="{$approveUrl}" title="{$approveText}">
                    <i class="fa fa-check-circle"></i> {$approveText}
                </button>

                <button class="btn btn-danger btn-sm reject-btn" data-url="{$rejectUrl}" title="{$rejectText}">
                    <i class="fa fa-times-circle"></i> {$rejectText}
                </button>
            </div>
        HTML;
        })->width(180);
    ...
    protected function detail($id)
    {
        ...
        $show->field('meta', __('Fields'))->unescape()->as(function () use ($withdrawal) {
            ...
            $html = '<div class="table-responsive"><table class="table table-bordered table-striped table-hover" style="width:100%;">';
            $html .= '<thead class="bg-light"><tr><th style="width:30%; padding:10px;">' . __('Field') . '</th><th style="padding:10px;">' . __('Value') . '</th></tr></thead><tbody>';
            ...
            foreach ($meta as $fieldId => $value) {
                ...
                $html .= sprintf(
                    '<tr><td style="padding:10px; vertical-align:middle;"><strong>%s</strong></td><td style="padding:10px;">%s</td></tr>',
                    e($title),
                    $valueFormatted
                );
            }
