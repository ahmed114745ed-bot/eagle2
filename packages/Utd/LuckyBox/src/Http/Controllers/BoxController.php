...
class BoxController extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        if (! $box) {
            return Common::apiResponse(0, 'box not found', null, 404);
        }
        if (($box->type === 0) && ! $request->users_num) {
            return Common::apiResponse(0, 'missing number of users', null, 422);
        }
        if ($user->di < $box->coins) {
            return Common::apiResponse(0, __('low balance'), null, 407);
        }
    ...
    {
        if (! $request->box_id || ! $request->room_uid) {
            return Common::apiResponse(0, 'missing params', null, 422);
        }
    ...
    public function superBox($box_use, $user, $keyBoxUse, $bosUserId)
    {
        ...
        return Common::apiResponse(1, 'you are in waiting list', ['type' => mb_strtolower(TypeBox::from($box_use['type'])->name)], 200);
    }
}
