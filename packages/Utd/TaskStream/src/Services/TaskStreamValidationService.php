...
class TaskStreamValidationService
{
    ...
    public function checkRoomsIds($allRoomIds, $liveRoomId): void
    {
        ...
        if (! empty($missing)) {
            throw new CValidationException(__('Some rooms are not live or do not exist: ').implode(',', $missing), ResponseAlias::HTTP_UNPROCESSABLE_ENTITY);
        }
        if (! in_array($liveRoomId, $allRoomIds)) {
            throw new CValidationException(__('Host must be part of the teams'), ResponseAlias::HTTP_UNPROCESSABLE_ENTITY);
        }
    }
