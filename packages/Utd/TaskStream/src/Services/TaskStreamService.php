...
use Utd\TaskStream\Entities\PkSession;
use Utd\TaskStream\Events\TaskStreamInvitationEvent;
use Utd\TaskStream\Events\TaskStreamRespondInvitationEvent;
use Utd\TaskStream\Repositories\TaskStreamInvitationRepository;
use Utd\TaskStream\Repositories\TaskStreamRepository;
use Utd\TaskStream\Repositories\TaskStreamRoomRepository;

class TaskStreamService extends TaskStreamValidationService implements TaskStreamServiceContract
{
    ...
    public function sendInvitation(array $data)
    {
        ...
        $invitationData = [
            ...
            'user_image' => $authUser->profile->avatar,
            'room_id' => $liveRoom->id,
            'task_stream_id' => $taskStream->id,
            'battle_data' => @$data['battle_data'],
        ];

        event(new TaskStreamInvitationEvent($inviteeUserId, $invitationData));
    ...
    public function respondInvitation(array $data)
    {
        ...
        if ($status === 'accept') {
            ...
            event(new TaskStreamRespondInvitationEvent($invitation->inviter_user_id, $invitationResponseData));
