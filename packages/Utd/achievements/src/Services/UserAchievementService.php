...
class UserAchievementService implements UserAchievementContract
{
    ...
    /**
     * Track room target achievements
     */
    ...
    /**
     * Track gift target achievements
     */
    ...
    /**
     * Get user's picked achievements
     */
    ...
    /**
     * Get room achievements for a user
     */
    public function roomAchievement(int $ownerId)
    {
        $roomAchievement = Achievement::query()
            ->where('type', AchievementType::ROOM_TARGET)
            ->with('levels:id,achievement_id,valid_image')
            ->first();
        ...
        $usersAchievements = UserAchievementLevel::query()
            ...
            ->where(fn($query) => $query->whereIn('achievement_level_id', $levelsIds)
                ->orWhere(fn($q) => $q->where('achievement_id', '=', null)
                    ->where('custom_image', '!=', null)))
            ->get();

        return $usersAchievements->map(function ($item) use ($levels) {
            return [
                'image' => $levels->where('id', $item->achievement_level_id)?->value('valid_image') ?? $item->custom_image
            ];
        });
    }
}
