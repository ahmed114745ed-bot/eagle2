...
class AgencyMangerTaregetController extends MainController
{
    ...
    protected function agencies()
    {
        ...
        $grid->model()->where('agency_dash_manger_id', $loggedInUserId);
        ...
        $grid->column('target', __('target'))->display(function () {
            return @$this->target()->sallary ?: 0;
        });
        $grid->column('expenses', __('expenses'))->display(function () {
            return @$this->target()->cut_amount ?: 0;
        });
        ...
        $grid->column('cashing1', __('u_Salary'))->display(function () {

            $fromconfig = Config::where('name', 'agency_manager_percentage')->first();
            $result = $this->salary * ($fromconfig->value / 100);
        ...
        $grid->tools(function (Grid\Tools $tools) {
            // $tools->append('<a href="' . route('agency-export-report', ['month' => request()->month, 'year' => request()->year, 'agency_id' => request()->id]) . '" target="_blank" class="btn btn-sm btn-success"><i class="fa fa-download"></i> Export Excel</a>');
            // $tools->append ('cashing',__ ('cashing'))->display (function (){
            //     $options = ['user'=>__('user')];
            //     return (new \App\Admin\Actions\SalaryAction($this->id,'user'))->render () ;
            // });
            $tools->append('<a href="'.route('agency-export-report').'" target="_blank" class="btn btn-sm btn-success"><i class="fa fa-download"></i> vvv</a>');
