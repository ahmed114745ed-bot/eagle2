...
class AgencyAppController extends Controller
{
    ...
    public function actionRequestAgency(Request $request)
    {
        ...
        if (! $agency) {
            AgencyHelper::apiResponse(0, 'agency not found', 404);
        }
        if ($request->status !== 1) {
    ...
    public function cancel_request_createAgency(Request $request)
    {
        ...
        if (! $agency) {
            return AgencyHelper::apiResponse(0, __('not found request'), []);
        }

        if ($agency->status !== 0 && $agency->status !== 3) {
            return AgencyHelper::apiResponse(0, __('not cancel request'), []);
        }
        if ($agency->status === 3) {
            return AgencyHelper::apiResponse(0, __('you already canceled'), []);
        }

        $time_after24 = Carbon::parse($agency->created_at)->addHours(24);
        if ($agency->created_at > $time_after24) {
            return AgencyHelper::apiResponse(0, __('24 hours have passed since your request'), []);
        }
    ...
    public function user_agency_information()
    {
        ...
        if (! $agency) {
            return AgencyHelper::apiResponse(0, __('api_responses.u_not_owner_agncy'), []);
        }
