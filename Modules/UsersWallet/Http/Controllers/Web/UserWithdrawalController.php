...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function handleAction(button, actionType) {
        ...
        button.addEventListener('click', function(e){
            ...
            const messages = {
                approve: {
                    title: '". __('messages.approve_title') ."',
                    confirm: '". __('messages.approve_confirm') ."',
                    cancel: '". __('messages.cancel') ."',
                    color: '#28a745'
                },
                reject: {
                    title: '". __('messages.reject_title') ."',
                    confirm: '". __('messages.reject_confirm') ."',
                    cancel: '". __('messages.cancel') ."',
                    color: '#dc3545'
                },
                ...
                if(result.value){
                    ...
                    sendRequest(url).then(res => {
                        if(res.success){
                            ...
                            Swal.fire('خطأ', res.message || messages.error, 'error');
                        }
                    }).catch(() => {
                        Swal.fire('خطأ', messages.error, 'error');
                    });
                }
            });
        });
