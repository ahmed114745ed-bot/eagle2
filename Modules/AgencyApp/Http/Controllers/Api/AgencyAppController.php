...
class AgencyAppController extends Controller
{
    ...
    public function agency_filter(Request $request)
    {
        ...
        $agencies = Agency::query()
            ...
            ->take(10)
            ->get();
        $agency_manger = User::query()->select(['*', DB::raw("((LENGTH(users.uuid) - LENGTH(REPLACE(users.uuid, '{$keyword}', ''))) / CHAR_LENGTH(users.uuid)) * 100 AS matching_percentage")])->has('ownAgency')->where(function ($q) use ($keyword) {
    ...
    {
        \Log::info('i am here in the wrong class');
        $user  = \Auth::user();
