...
class UserController extends MainController
{
    ...
    protected function grid()
    {
        ...
        // Fix dropdown clipping for last rows on mobile
        Admin::script(<<<'JS'
        $(document).on('click', '.grid-action-toggle', function(e) {
            ...
            var l = r.right - mw; if(l<5) l=5; if(l+mw>ww) l=ww-mw-5;
            var t = up ? (r.top - mh - 2) : (r.bottom + 2);
            $c.css({display:'block',position:'fixed',top:t+'px',left:l+'px',zIndex:99999,
                minWidth:'160px',background:'#fff',borderRadius:'4px',
                border:'1px solid rgba(0,0,0,.15)',boxShadow:'0 6px 20px rgba(0,0,0,.18)',
