...
//        try {
    ...
    public function showAgencyRequest(Request $request)
    {
        ...
        if ($type == "application") {
            $list_req1 = $list_req->where('status', 0)->with('user')->paginate($request->perPage, ['*'], 'page', $request->page);
            $list_req = MyDataForAgencyNewResource::collection($list_req1, 'application');
        } elseif ($type == "record") {
            $list_req = $list_req->where('status', '!=', 0)->with('user','admin')->paginate($request->perPage, ['*'], 'page', $request->page);
