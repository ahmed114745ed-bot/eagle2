...
if (!function_exists('get_file_details')) {
    ...
    if (!function_exists('dispatchRoomsRedis')) {
        ...
        function dispatchRoomsRedis(int $roomId, int $userId, $coins = 0, $data = null, string $type = 'charisma'): void
        {
            ...
            \Illuminate\Support\Facades\Log::channel('daily')->info('[CHARISMA][1-DISPATCH] dispatchRoomsRedis called', [
                'room_id'  => $roomId,
                'user_id'  => $userId,
                'coins'    => $coins,
                'type'     => $type,
            ...
            try {
                ...
                \Illuminate\Support\Facades\Log::channel('daily')->info('[CHARISMA][1-DISPATCH] Saved to Redis', [
                    'room_id'   => $roomId,
                    'user_id'   => $userId,
                    'redis_key' => $key,
                    'coins'     => $coins,
                ...
                \Illuminate\Support\Facades\Log::channel('daily')->error('[CHARISMA][1-DISPATCH] Redis save FAILED', [
                    'room_id' => $roomId,
                    'user_id' => $userId,
