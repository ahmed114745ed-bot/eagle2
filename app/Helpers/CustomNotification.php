...
class CustomNotification
{
    ...
    public function senderLevel(int $userId)
    {
        ...
        $icon = $data;
        Common::send_firebase_notification($tokens_notfacion, __('api.tik_chat'), $firebaseBody, icon: $icon, data: $data);
    ...
    public function BackgroudRequest(User $user ,$type=0)
    {
        ...
        $firebaseBody = ($user->lan === 'ar') ? $body_ar : $body_en;
        Common::send_firebase_notification($tokens_notification, __('api.Soalfna'), $firebaseBody,messageType: 'backgroud-request' );
    ...
    public function target(int $userId)
    {
        ...
        Common::send_firebase_notification($tokens_notfacion, __('api.tik_chat'), $firebaseBody,data: $data,  messageType: 'achieve-target-monthly');
    ...
    public function acceptAgency(Agency $agency, User $user)
    {
        ...
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody);
    ...
    public function visitProfile(User $user, User $visitor)
    {
        ...
        $data['user_id'] = $user?->id;
        $icon = $data['image'];
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody, icon:$icon, data:$data, messageType: 'visit-profile' );
    ...
    public function removeFamilyUser(Family $family, User $user)
    {
        ...
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody, $icon);
    ...
    public function acceptRequestAgency(User $user)
    {
        ...
        $title_ar =  __('api.tik_chat',[],'ar');
        $title_en =  __('api.tik_chat',[],'en');
        $title               = ($user?->lan === 'ar') ? $title_ar : $title_en;
        Common::send_firebase_notification($tokens_notification, $title, $firebaseBody,messageType: 'accept-agency');
    ...
    public function sendMomentGift(User $senderUser, Gift $gift, $receivedUser,$momentId)
    {
        ...
        Common::send_firebase_notification($tokens_notfacion, __('api.tik_chat'), $firebaseBody, icon: $icon, data: $data,messageType: 'send-moment-gift');
    ...
    public function mallSend($user, $toUser, $type, $bubbleImage)
    {
        ...
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody, icon: $icon, data: $data,messageType: 'mall-send');
    ...
    public function acceptUserFamily(Family $family, ?User $user)
    {
        ...
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody, $icon,data: $data,messageType: 'accept-user-family');
    ...
    public function requestJoinFamily(Family $family, User $user)
    {
        ...
        $data['family_id']=$family->id;
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody, $icon, data: $data,messageType: 'request-join-family');
    ...
    public function family(Family $family, User $user)
    {
        ...
        $icon = $family->img;
        $data['family_id']=$family->id;
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody, $icon,data: $data,messageType: 'family');
    ...
    public function acceptAgencyApp(Agency $agencyName, User $user)
    {
        ...
        $icon = $agencyName->img;
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody, $icon, $data,messageType: 'accept-agency-app');
    ...
    public function chargeAction(User $user, $request)
    {
        ...
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody,data:$data ,messageType: 'charge-action-notifaction');
    ...
    public function banUser(User $user, $duration)
    {
        ...
        $firebaseBody        = ($user?->lan === 'ar') ? $body_ar : $body_en;
        $data['user_id'] = $user?->id;
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody,data: $data,messageType: 'ban-user');
        Common::sendOfficialMessage($user->id, $body_en, '', titleAr: $body_ar);
        (new UserCounterServices)->eventUser($user,'official-messages');
    ...
    public function removeBanUser(User $user)
    {
        ...
        $firebaseBody        = ($user?->lan === 'ar') ? $body_ar : $body_en;
        $data['user_id']=$user?->id;
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody,data:$data,messageType: 'remove-ban-user');
        Common::sendOfficialMessage($user->id, $body_en, '', titleAr: $body_ar);
        (new UserCounterServices)->eventUser($user,'official-messages');
    ...
    public function vips(User $user, $duration, $img)
    {
        ...
        $icon = $data['image'];
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody, icon: $icon, data: $data,messageType: 'vips');
        Common::sendOfficialMessage($user->id, $body_en, '', titleAr: $body_ar);
        (new UserCounterServices)->eventUser($user,'official-messages');
    ...
    public function familyLevelUpgrade(int $familyId)
    {
        ...
        Common::send_firebase_notification($usersTokenEn, __('api.tik_chat', locale: 'en'), $body_en, icon: $icon, data: $data,messageType: 'family-level-upgrade');
        Common::send_firebase_notification($usersTokenAr, __('api.tik_chat', locale: 'ar'), $body_ar, icon: $icon, data: $data,messageType: 'family-level-upgrade');
    }
    ...
    public function wareVip(User $user, $duration, $name, $image)
    {
        ...
        $icon = $data['image'];
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody, icon: $icon, data: $data,messageType: 'ware-vip');
        Common::sendOfficialMessage($user->id, $body_en, '', titleAr: $body_ar);
        (new UserCounterServices)->eventUser($user,'official-messages');
    }
    ...
    public function sendNotificationWithImage(mixed $momentUser, string $body_ar, string $body_en, User $userSender, string $title,string $messageType,$data): void
    {
        ...
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody, $icon, $data,messageType: $messageType);
    ...
    public function acceptRequestToGetMony(User $user,$type= 1,$reason = null,$value=0)
    {
        ...
        $firebaseBody        = ($user?->lan === 'ar') ? $body_ar : $body_en;
        $title               = ($user?->lan === 'ar') ? $title_ar : $title_en;
        Common::send_firebase_notification($tokens_notification, $title, $firebaseBody, icon: '', data: [],messageType: 'request-message');
        Common::sendOfficialMessage($user->id, $body_ar, '', titleAr: $body_ar);
        (new UserCounterServices)->eventUser($user,'official-messages');
    ...
    public function roomAchievementTarget(User $user,$coins, $roomName)
    {
        ...
        $firebaseBody        = ($user?->lan === 'ar') ? $body_ar : $body_en;
        $data['user_id']=$user?->id;
        Common::send_firebase_notification($tokens_notification, __('api.tik_chat'), $firebaseBody,data:$data,messageType: 'room-achievement-target');
        Common::sendOfficialMessage($user->id, $body_en, '', titleAr: $body_ar);
        (new UserCounterServices)->eventUser($user,'official-messages');
    }
}
