...
class AgencyRecoveryJob implements ShouldQueue
{
    ...
    protected $reportFile = 'agency_recovery_report.html';
    ...
    public function handle()
    {
        ...
        if (!$debtor) {
            $this->finishReport();
            Log::info("AgencyRecovery: All agency debtors processed.");
            return;
        }
        ...
        try {
            // Get all charges by this agency (charger_type = host_agency)
            ...
            foreach ($charges as $charge) {
                ...
                if ($charge->user_type === 'agency') {
                    ...
                    // If not enough di, trace user's gifts recursively
                    if ($chargeAmount > 0) {
            ...
            // Mark affected users for salary recalc
            ...
            // Mark agency debtor as processed if still negative
            if ($remaining > 0) {
            ...
            $this->appendRow($debtor->agency_id, $agencyName, $debtUsd, $recoveredUsd, $remaining / $zonesCoins, $status);

            Log::info("AgencyRecovery: Processed agency {$debtor->agency_id}. Recovered: {$recoveredCoins} coins.");
            ...
            if ($attempt < $maxRetries && str_contains($e->getMessage(), 'Deadlock')) {
                Log::warning("AgencyRecovery: Deadlock for agency {$debtor->agency_id}, retry {$attempt}/{$maxRetries}");
                $this->traces = [];
            ...
            $this->appendRow($debtor->agency_id, $agencyName, $debtUsd, 0, $debtUsd, 'error', $e->getMessage());
            Log::error("AgencyRecovery: Failed for agency {$debtor->agency_id}: " . $e->getMessage());
...
<title>Agency Recovery Report</title>
<style>
...
HTML;
    ...
    private function appendRow(int $agencyId, string $agencyName, float $debt, float $recovered, float $stillNegative, string $status, string $errorMsg = ''): void
    {
        $this->initReport();
        $path = public_path($this->reportFile);
        ...
        $agencyName = htmlspecialchars($agencyName, ENT_QUOTES, 'UTF-8');
        $cardId = "agency-{$agencyId}-" . time();
...
<span class="user-id">Agency #{$agencyId}</span>
<span class="user-name">{$agencyName}</span>
</div>
